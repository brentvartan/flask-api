...
def register_commands(app):
    ...
    def rederive_tiers(dry_run):
        """Recompute watch_level for scored signals under the current floor rules.

        Shares its implementation with POST /api/admin/rederive-tiers — see
        enrichment.rederive_watch_levels. Free: no Anthropic calls.
        """
        from .services.enrichment import rederive_watch_levels
        counts = rederive_watch_levels(dry_run=dry_run)
        prefix = "DRY RUN — would change" if dry_run else "✓ Re-derived"
        click.echo(f"{prefix}: {counts}")
